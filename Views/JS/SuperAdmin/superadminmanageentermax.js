...
function searchSTU(url){
    ...
    let url22 = "?type=searchapplyentermax&target=stu&depart="+department+"&class="+class_name+"&number="+searchnumber;
    let requestForResult = new XMLHttpRequest();
    requestForResult.onreadystatechange = function () {
        if (requestForResult.readyState == 4 && requestForResult.status == 200) {
            document.getElementById("maxresult").innerHTML = requestForResult.responseText;
        }
    }
